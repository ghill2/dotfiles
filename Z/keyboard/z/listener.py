...
class KeyEventListener(object):
    ...
    def run(self):
        ...
        return
        loopsource = Quartz.CFMachPortCreateRunLoopSource(None, self.tap, 0)  # segfault here
    ...
    def handler(self, proxy, e_type, event, refcon):
            ...
            for bitmask, key_name_suffixes in (
                ...
                if ends_with_suffix:
                    ...
                    key_name_suffix = key_name_suffixes[
                        0]  # it doesn't matter here if we clobber suffixes from the same modifier like option/alt
        ...
        self.callback(
            KeyboardEvent(event_type, scan_code, name=key_name, is_keypad=is_keypad, modifiers=pressed_modifiers_tuple))
        return event
